...
typedef  enum AudioFrameType {
    AudioFrameTypeRecord = 1000,
    AudioFrameTypePlayout,
    AudioFrameTypeRecordPlayout,
    AudioFrameTypeRecordPlayoutMixing,
    AudioFrameTypeOthers
}AudioFrameType;
...
/**
 * Called when the audio device get record or playout data. The argument is sample rate,
 * and audio type whic is record, playout or mixer.
 */
- (void)audioSession:(RTCAudioSession *)audioSession
        getAudioData:(NSData*)audioFrame
          sampleRate:(int)sampleRate
       numOfChannels:(int)numOfChannels
            bitDepth:(int)bitDepth
           audioType:(AudioFrameType)audioType;
...
/**
 * Called when playout audio pause
 */
- (void)didAudioPlayPauseStart:(RTCAudioSession *)audioSession;

- (void)didAudioPlayPauseStop:(RTCAudioSession *)audioSession;
...
/** This property is used when exposing audio buffer to user.
 *  The audio buffer may be recording type, playout type, or mixing type.
 */
@property (nonatomic, assign) AudioFrameType audioFrameType;

/** This property is used to make output silence
 true: output silence
 */
@property (atomic, assign) BOOL isAudioSpeakerMute;

/** This property is used to make recorded micphone data silence when using mute interface
 true: micphone data silence
 */
@property (atomic, assign) BOOL isMicphoneMute;
...
/*get micphone data*/
- (void)getRecordedData:(void*)audioData
               dataSize:(UInt32)dataSize
                 sampleRate:(UInt32)sampleRate
              numOfChannels:(UInt32)numOfChannels
                    bitDepth:(UInt32)bitDepth
                   timeStamp:(UInt64)timeStamp;

/*get playout data*/
- (void)getPlayoutData:(void*)audioData
               dataSize:(UInt32)dataSize
             sampleRate:(UInt32)sampleRate
          numOfChannels:(UInt32)numOfChannels
               bitDepth:(UInt32)bitDepth;

- (void)didAudioPlayPauseStart;

- (void)didAudioPlayPauseStop;
